...
public class GroupDashboard extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        iv_actionbtn.setImageResource( R.drawable.ic_person_blue );//(R.drawable.overflow_btn_blue);//ic_person_blue
        ...
        if (isAdmin.equalsIgnoreCase( "Yes" )) {
            ...
            // added by satish on 03-06-2019 because admin module should show only admin
            for (ModuleData moduleData : listmodules) {

                String moduleId = moduleData.getModuleId();

                if (moduleId.equalsIgnoreCase( Constant.Module.ADMIN )) {
                    listmodules.remove( moduleData );
                    break;
                }
            }

            //Error Comes added By Gaurav (display offline Stored Data)
            //Take to much time to display Data for that use retryDataDisplay() Method
...
//        Intent service = new Intent( getApplicationContext(), DirectorySyncService.class );
