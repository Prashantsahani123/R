            ...
            while ( cursor.moveToNext() ) {
                ...
                if ( list.indexOf(md) == -1) {
                    ...
                    if(moduleId.equalsIgnoreCase(Constant.Module.SUB_GROUPS)){
                        ...
                        // comment by satish on 22 May 2019 disscued in meeting on 17-05-2019
    ...
    public boolean syncData(long masterUid, ArrayList<ModuleData> newRecords, ArrayList<ModuleData> updatedRecords, ArrayList<ModuleData> deleteRecords) {
        ...
        try {
            ...
            while ( deletedIterator.hasNext() ) {
                ...
                Log.i("delete","**************** Delete module =>"+deleted);
                if ( ! deleted ) {
                    db.endTransaction();
                    return false;
                }
   ...
   /* public boolean updateModuleMaster(long masterUid, ModuleData md) {
    ...
    public boolean deleteModuleMasterModel(long masterUid,ModuleData data) {
        ...
        try {
            ...
            boolean  dataAvailable = moduleExists(masterUid,data.getGroupModuleId(),data.getGroupId(),data.getModuleId());

            if(dataAvailable) {
                n = db.delete(Tables.ModuleDataMaster.TABLE_NAME,  "masterUID=? and groupModuleId=? and groupId=? and moduleId=?", new String[]{"" + masterUid, "" + data.getGroupModuleId(),""+data.getGroupId(),""+data.getModuleId()});
    ...
    public boolean moduleExists(long masterUid,String grpModuleId,String groupId,String moduleId) {
        ...
        try {
            ...
            String query = "select * from module_data_master where masterUID=? and groupModuleId="+grpModuleId+" and groupId="+groupId+" and moduleId="+moduleId;

            Log.e("query run", "query =>"+query);

            Cursor cursor = db.rawQuery(query, new String[]{""+masterUid});
