...
public class ReplicaInfoModel {
    ...
    public String getReplicaOf(String moduleId) {
        try {
           ...
           // Log.e("Touchbase","replica Query => select "+Tables.ReplicaInfo.Columns.REPLICA_OF+" from "+Tables.ReplicaInfo.TABLE_NAME+" where moduleId=?"+moduleId);

            if ( cursor.moveToNext() ) {
                String replicaOf = cursor.getString(0);

//                Log.d("Touchbase","replica value =>"+replicaOf);
                cursor.close();
